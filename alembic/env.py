...
def do_run_migrations(connection):
    context.configure(
        ...
        target_metadata=target_metadata,
        compare_type=True,  # detecta mudancas de tipo nas colunas
    )
    with context.begin_transaction():
        context.run_migrations()
...
async def run_migrations_online() -> None:
    """
    Modo online com engine async (asyncpg).
    run_sync envolve o callback síncrono do Alembic dentro da conexão async.
    """
